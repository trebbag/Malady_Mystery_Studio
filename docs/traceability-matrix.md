...
| Local review and approvals | `approval-decision`, `canonicalization-resolution`, `review-comment`, `review-assignment`, `artifact-diff-view`, `audit-log-entry`, `workflow-run`, `qa-report`, `eval-run`, `clinical-package-view`, `review-run-view` | governance release + evidence traceability + panelization + render readiness | T005, T304, T401 | Gate 4, Gate 6 |
...
| Frontend placeholder and handoff prep | `review-dashboard-view`, `review-run-view`, `clinical-package-view`, `evaluation-summary-view`, `export-history-view`, `source-governance-view`, `trace-coverage-view`, `artifact-diff-view` | none directly; backed by the same run/eval artifacts | T304 follow-through | n/a |
